...
onload = function () {
    ...
    var url = 'https://demos.componentone.com/ASPNET/c1webapi/4.0.20173.114/api/dataengine/';
    app.dataSets = [

        // server (via WebAPI server)
        ...
        // client (local data)
        { name: 'Client (100 items)', value: getDataSet(100), server: false },
        { name: 'Client (50k items)', value: getDataSet(50000), server: false },
        { name: 'Client (100k items)', value: getDataSet(100000), server: false },
        { name: 'Client (500k items)', value: getDataSet(500000), server: false },
    ];
    ...
    app.cmbDataSets = new wijmo.input.ComboBox('#cmbDataSets', {
        ...
        formatItem: function (s, e) {
            e.item.innerHTML = e.data.server
                ? '&#9729; ' + e.data.name
                : e.data.name;
        }
    });
    ...
    app.propertyChanged.addHandler(function (s, e) {
        ...
        switch (e.propertyName) {
            case 'data':
                ...
                // use custom set of fields for the cube sources
                if (newVal == url + 'cube' || newVal.cube) {
                    ...
                    if (!ng.autoGenerateFields) { // clear non-auto-generated fields (TFS 245470)
                        ng.fields.clear(); 
                    }
                ...
                ng.itemsSource = newVal;
                break;
            case 'showRowTotals':
                ng.showRowTotals = newVal;
                break;
            case 'showColTotals':
                ng.showColumnTotals = newVal;
                break;
            case 'showZeros':
                ng.showZeros = newVal;
                break;
            case 'totalsBeforeData':
                ng.totalsBeforeData = newVal;
                break;
        }
    });
    ...
    app.propertyChanged.addHandler(function (s, e) {
        switch (e.propertyName) {
            case 'data':
                ...
                // update items source
                var server = ng.collectionView == null;
                app.rawGrid.itemsSource = ng.collectionView;

                // check whether the data is read-only
                app.rawGrid.isReadOnly = server;
    ...
    app.export = function () {
        ...
        // save the transposed and raw data views only for local data (TFS 321538)
        if (ng.itemsSource instanceof wijmo.collections.CollectionView || wijmo.isArray(ng.itemsSource)) {
...
function transposeView(ng) {
    ng.deferUpdate(function () {
        ...
        for (var r = 0; r < ng.rowFields.length; r++) {
            rows.push(ng.rowFields[r]);
        }
        for (var c = 0; c < ng.columnFields.length; c++) {
            cols.push(ng.columnFields[c]);
        }
