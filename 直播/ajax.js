...
function selectSingleNode(xmlDoc, elementPath){//获取SingleNode对象: xmlDoc:XmlDOM, elementPath:指定路径
	if(window.ActiveXObject){
		...
		try{
			...
			var IE8nodevalue = FormatXmlDOMToString(xmlDoc)	
			//alert("selectSingleNode == "+IE8nodevalue);
			return getnodebyelementpath(IE8nodevalue,elementPath);
		}
	}
}
...
function selectSingleNodeValue(xmlDoc, elementPath){//获取SingleNode值: xmlDoc:XmlDOM, elementPath:指定路径
	if(window.ActiveXObject){return xmlDoc.selectSingleNode(elementPath).text;}
		...
		try{
			...
			var IE8nodevalue = FormatXmlDOMToString(xmlDoc)	
			//alert("selectSingleNodeValue == "+IE8nodevalue);
			return getnodebyelementpath(IE8nodevalue,elementPath);
		}
	}
}
...
//将xml格式转化为字符串
function FormatXmlDOMToString(DOMxmlDoc)
{
	if (window.ActiveXObject) {
		return DOMxmlDoc.xml;
	}
	else {
		var oSerializer1 = new XMLSerializer();
		return oSerializer1.serializeToString(DOMxmlDoc);
	}
}
function getnodebyonepath(doc,elementPath)
{
	try{
		return doc.split("<"+elementPath+">")[1].split("</"+elementPath+">")[0];
	}catch(e){
		return "";
	}
}
	
function getnodebyelementpath(doc,elementPath)
{
	try{
		var myelementPaths = elementPath.split("/");
		for(i=0; i<myelementPaths.length; i++)
		{
			doc = getnodebyonepath(doc,myelementPaths[i]);
		}
		return doc;
	}catch(e){
		return "";
...
function getselectnodes(doc,path)
{
	try{
		var value = doc.split("<"+path+">");
		var i = 1;
		for(i=1; i<value.length; i++)
		{
			value[i-1]=value[i].split("</"+path+">")[0];			
		}
		return value;
	}catch(e){
		return "";
   ...
   if (oSend.readyState == 4) {
        if (oSend.status == 200){
			...
			try{
				...
				switch(paramFlag){
				case 10:		//单参数处理
					returnCmd 	= parseInt(selectSingleNodeValue(xmlResult,"html/body/cmd"));
