...
function onReBack(a){
	...
	if(a=="onmouseUp"){
		var time = rSV;
		var nWindow = parseInt($("nwindows").value);
		var movetime=parseInt(time/100*nTotalPlayTime);
		$("ReVideoX").PLAY_Seek(nWindow, movetime);
		//alert("time:"+time+"movetime:"+parseInt(rSV/100*nTotalPlayTime));
	}
}
...
function funSearch(){
	$("sellist_span").innerHTML ="<select name=\"sellist\" size=\"15\" class=\"text\" id=\"sellist\" style=\"width:180px;\" onDblClick=\"funReplayFile(this.value);\"></select>";<!--<span id="sellist_span"> 用于兼容IE11下文件列表不可点击-
	var bStopSearch;
	...
	var szNVSName 	= "DVS";
	var szURL 		=$("url").value;
	var pagePort1=$("port").value;
	var nSvrPort 	= parseInt(pagePort1);
//	var szUserName 	= pageUNAME;
	//var szUserPwd 	= pagePASSWORD;
	//alert(pageUNAME+pagePASSWORD);
	var szUserName =$p("username").value;
    var szUserPwd=$p("password").value;
...
function funDownloadFile(){
	...
	var szFileNames = $("sellist").options[nListIndex].text;
...
function funReplayFile(CMDVALUE){
	...
	default:
  	...
  	var szFileNames = $("sellist").options[$("sellist").selectedIndex].text;
	var szFileNameArr = new Array();
	var szFileName;
