...
function initvideo(flag){
	...
	$s("WebCMS").m_szUserName	= $p("username").value;
	$s("WebCMS").m_szPassword	= $p("password").value;
...
function sumbitselclr(){	//清除屏蔽区域
  try{
	...
	var i;
	for(i=1;i<=4;i++){
		var p =document.getElementById("nLeft"+i);
		p=0;
		var a =document.getElementById("nTop"+i);
		a=0;
		var b =document.getElementById("nWidth"+i);
		b=0;
		var c =document.getElementById("nHeight"+i);
		c=0;
	}
	return false;
}
