...
var USERCHECKSTRING1   = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ`~!@#$%^&*()-_+=|{}[];:'<>?,./\\\"";
...
if(top.location.href == self.location.href){
	...
	pageUNAME		= getCookies("pagerUseName");
	pagePASSWORD	= getCookies("stringmvc");
	pagePort		= parseInt(getCookies("pagePort"));
	pageURL			= getDocURL();		//getCookies("pageURL");
}else{
	if($p("bCenterWeb").value=="1"){ //中性模式
		pageUNAME		= $p("uname").value;	//getCookies("pageUname");
		pagePASSWORD	= $p("passwd").value;	//getCookies("pagePassword");
	}else{
		pageUNAME		= getCookies("pagerUseName");	//getCookies("pageUname");
		pagePASSWORD	=  getCookies("stringmvc");	//getCookies("pagePassword");
	}
	pagePort		= parseInt(getCookies("pagePort"));	//parseInt(getCookies("pagePort"));
	pageURL			= getDocURL();		//getCookies("pageURL");
...
if(pagePASSWORD==null||pagePASSWORD=="null"||pagePASSWORD=="undefined"){pagePASSWORD="admin";}//|
