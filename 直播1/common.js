...
function dowebstype(value,pram)
{
var stype = getwebtype();
  switch(parseInt(value)){
   case 0:      //閲婃斁鍐呭瓨 CollectGarbage(); 
  if("ie" == stype)
  {
    CollectGarbage(); 
  }else if("hh" == getwebtype())
  {}
    break;
  case 1:      //灏嗗ぇ鍐欒浆鍖栦负灏忓啓 CollectGarbage(); 
  if("ie" == stype)
  {
    CollectGarbage(); 
  }else if("hh" == getwebtype())
...
if(returnURL=="127.0.0.1"){returnURL = "192.168.1.231";}	//panch 璋冭瘯锛岀洿鎺ヨ繑鍥炲彲鐢↖P.
return returnURL;
}
//璇诲彇Cookies
...
var userId=null;
for(var i=0;i<arrCookie.length;i++){	//閬嶅巻cookie鏁扮粍锛屽鐞嗘瘡涓猚ookie瀵�
   var arr=arrCookie[i].split("=");
   if(strName==arr[0]){userId=unescape(arr[1]); break;}	//鎵惧埌鍚嶇О涓簎serId鐨刢ookie锛屽苟杩斿洖瀹冪殑鍊�
}
return(userId);
}
//璁剧疆Cookies
...
/*********************************
//flag:	1妫€娴婭P  2妫€娴嬪煙鍚�  3妫€娴婭P鎴栧煙鍚�
*********************************/
...
//鍒ゆ柇鏄惁涓烘暟缁� 
//typeof 杩斿洖鍊兼湁鍏鍙兘锛� "number," "string," "boolean," "object," "function," 鍜� "undefined."
...
function check_com(theForm,checkOK,Msg){     //鍏敤
try{
...
function setDisplay(a, c){
if(!$(a))return;
c?$(a).style.display="":$(a).style.display="none"
}
