// JavaScript Document
var bRECORD = new Array(0,0,0,0), bCALL, bLISTEN = new Array(0,0,0,0), bSCALE, bORIGINAL,bREGIONAL; 
...
console.log("好成绩看下次开机按每次看")
function ocxConfirm(){
	console.log(1156261)
	try{
		console.log(1515611)
		window.location.href='C:/Windows/System32/WebCMS.ocx';
		// console.log(window.location.href)
		if($("WebCMS").attr("StartPreview") == undefined) {
			console.log(15418551)
			if(window.confirm("ALERTOCX") == true) {
				console.log(154651)
				window.location.href='/WebCMS.exe';
			}
		}
	}catch(e){}
...
function changeStream(CTRL_OBJ, CTRL_CMD){
	...
	console.log(233334444)
	if(bRECORD[nWindows]){
		if(window.confirm(ALERTVIEWRECORD+ALERTVIEWCHANGE)== false) {
			return false;
		}else{
...
function funRegional(CTRL_OBJ, CTRL_CMD){
	if(!bREGIONAL && !CTRL_CMD){
		CTRL_OBJ.className	= "leftbtn2 regional1";
		bREGIONAL	= 1;
		
		if($("baLineorRegional").value == 1)
		{
			OninitDrawEx('1');
		}
		else if($("baLineorRegional").value == 2)
		{
			OninitDrawEx('4');
		}
		
	}else{
		CTRL_OBJ.className = "leftbtn2 regional";
		bREGIONAL	= 0; 
		sumbitselclr();
	}
...
function funPTZCtrl(CTRL_OBJ, CTRL_CMD){
	...
	if(!RIGHT_PTZ_SHOW && !CTRL_CMD){
		...
		if(!b_INIT_PTZ){
			...
			ChangeLanguage('../xml/ptz.xml', m_szLanguage);
			
			$("yt1").title=getNodeValue('b_yt1');
			$("yt2").title=getNodeValue('b_yt2');
			$("yt3").title=getNodeValue('b_yt3');
			$("yt4").title=getNodeValue('b_yt4');
			$("yt5").title=getNodeValue('b_yt5');
			$("yt6").title=getNodeValue('b_yt6');
			$("yt7").title=getNodeValue('b_yt7');
			$("yt8").title=getNodeValue('b_yt8');
			$("yt9").title=getNodeValue('b_yt9');
		}
	}else{
...
function autoScaleWH(V_W, V_H, C_W, C_H){
	...
	if(ImageWidth/ImageHeight >= BodyWidth/BodyHeight){
		if(ImageWidth > BodyWidth){
			tmpWidth = BodyWidth;
			tmpHeight = (ImageHeight*BodyWidth)/ImageWidth; 
		}else{
			tmpWidth = ImageWidth; 
		...
		if(ImageHeight > BodyHeight){
			tmpWidth = (ImageWidth*BodyHeight)/ImageHeight;
			tmpHeight = BodyHeight; 
		}else{
			tmpWidth = ImageWidth; 
			tmpHeight = ImageHeight;
		} 
	...
	if(ImageHeight < BodyHeight){
		$("video1").setStyle("paddingTop", ((BodyHeight-ImageHeight)/2) + "px");
	}
...
function initPTZCtrl(){
	...
	if(typeof(ptzbc)!="object"){ptzbc=new Slider($("bcx"), $("bck"), {
					steps:100, wheel:!0, onChange:function(a){gca=a; $("bck").setProperty("title", gca); $("t_bcn").setProperty("text", gca); setCookies("nBCValue", gca, 1); }});
	}
...
function initState(CTRL_CMD){
	...
	if(!CTRL_CMD){
		...
		$("b_regional").className	= "leftbtn2 regional";	//区域
	  ...
	  try{
		...
		if(bREGIONAL){
			$("b_regional").className	= "leftbtn2 regional1";
		}else{
			$("b_regional").className	= "leftbtn2 regional";
		}
		if(bORIGINAL){
