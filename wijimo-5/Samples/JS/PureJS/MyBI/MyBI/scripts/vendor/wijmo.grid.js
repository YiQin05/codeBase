    ...
    (function (grid) {
        ...
        var FlexGrid = /** @class */ (function (_super) {
            ...
            function FlexGrid(element, options) {
                ...
                /*internal*/ _this._vtRows = 0; // row virtualization threshold
                /*internal*/ _this._vtCols = 0; // column virtualization threshold
                //#endregion
                //--------------------------------------------------------------------------
                //#region ** events
                /**
                    ...
                    this.rows.deferUpdate(function () {
                        ...
                        for (var r = firstRow; r <= lastRow && r > -1 && r < pCells.rows.length; r++) {
                            ...
                            if (row.isVisible) {
                                ...
                                // headers
                                if (_this.autoSizeMode & grid.AutoSizeMode.Headers) {
                                    max = _this._getDesiredRowHeight(pHdr, r, eMeasure, cache);
                                }
                                // cells
                                if (_this.autoSizeMode & grid.AutoSizeMode.Cells) {
                                    max = Math.max(max, _this._getDesiredRowHeight(pCells, r, eMeasure, cache));
                                }
                                // update size
                                max += extra;
                                pCells.rows[r].height = max > pCells.rows.defaultSize ? max : null;
                            }
                        }
                        // done with measuring element
                        _this.cellFactory.disposeCell(eMeasure);
            ...
            FlexGrid.prototype.scrollIntoView = function (r, c, refresh) {
                ...
                if (r > -1 && r < this._rows.length && r >= this._rows.frozen) {
                    var row = this._rows[r], pct = this.cells.height > hei ? Math.round(row.pos / (this.cells.height - hei) * 100) / 100 : 0, offsetY = Math.round(this._maxOffsetY * pct), rpos = row.pos - offsetY, rbot = rpos + row.renderSize - 1; // TFS 273979, 316887
                    if (rbot > hei - sp.y) {
                ...
                if (c > -1 && c < this._cols.length && c >= this._cols.frozen) {
                    var col = this._cols[c], rgt = col.pos + col.renderSize - 1; // TFS 273979, 316887
                    if (rgt > -sp.x + wid) {
            ...
            FlexGrid.prototype._setFocus = function (force) {
                if (this.hostElement) {
                    if (force || !this.containsFocus()) {
                        var ae = wijmo.getActiveElement(), aedt = this.activeEditor, acell = this._activeCell, efocus = this._eFocus;
                        if (aedt) {
                            ...
                            if (!wijmo.contains(acell, ae)) {
                                if (ae != this._root) {
                                    ...
                                    if (!done) {
                                        acell.tabIndex = this._orgTabIndex;
                                        acell.focus(); // TFS 299364
                                    }
                                    efocus.tabIndex = -1;
                                }
                            }
                            ...
                            if (!wijmo.contains(efocus, ae) && ae != this._root) {
                                efocus.tabIndex = this._orgTabIndex;
                                efocus.focus();
                            }
                        }
            ...
            FlexGrid.prototype._setFocusNoScroll = function (e) {
                ...
                if (wijmo.supportsFocusOptions()) {
                    ...
                    var sp = this.scrollPosition, st = e.style, pos = st.position, opy = st.opacity;
                ...
                // make sure the host elements didn't scroll (TFS 265197)
                this._fixScroll();
            };
            ...
            FlexGrid.prototype._getDefaultRowHeight = function () {
                ...
                var eMeasure = this._eFocus, eTmp = null;
                if (!eMeasure.scrollHeight) {
                    eTmp = wijmo.createElement('<div class="wj-flexgrid"><div class="wj-cell">0</div></div>', document.body);
                    eMeasure = eTmp.children[0];
                }
                // get row height taking styles/padding into account
                var defRowHei = eMeasure.scrollHeight;
                if (isNaN(defRowHei) || defRowHei <= 6) {
                    defRowHei = 28;
                }
            ...
            FlexGrid.prototype._getDesiredRowHeight = function (panel, r, eMeasure, cache) {
                ...
                for (var c = 0; c < panel.columns.length; c++) {
                    ...
                    if (col.isVisible) {
                        var height = void 0;
                        var rng = this.getMergedRange(panel, r, c, false);
                        var key = {
                            ct: panel.cellType,
                            col: c,
                            rng: rng && rng.rowSpan > 1 ? rng.rowSpan : 1,
                            content: col.dataType == wijmo.DataType.Number ? '1' : panel.getCellData(r, c, true)
                        };
            ...
            FlexGrid.prototype._bindGrid = function (full) {
                ...
                // NOTE: forcing sync only on full updates (see VirtualData sample)
                if (this._cv) {
                    this._syncSelection(full);
            ...
            /*protected*/ FlexGrid.prototype._cvCollectionChanged = function (sender, e) {
                ...
                switch (e.action) {
                    ...
                    // this will also update aggregates and edit/validation indicators
            ...
            FlexGrid.prototype._updateContent = function (recycle, state) {
                var root = this._root, host = this.hostElement, cellHost = this.cells.hostElement, ae = wijmo.getActiveElement(), elFocus = wijmo.contains(host, ae) ? ae : null, oldActiveCell = this._activeCell;
                ...
                if (this.showMarquee) {
                    ...
                    if (!this.isRangeValid(sel)) {
                        ...
                        wijmo.setCss(marquee, {
                            left: rcm.left + cellHost.offsetLeft - dx / 2,
                            top: rcm.top + cellHost.offsetTop - dy / 2,
                            width: rcm.width + dx,
                ...
                if (elFocus) {
                    if (elFocus != root &&
                        ...
                        wijmo.contains(host, elFocus) &&
                        !wijmo.contains(cellHost, elFocus)) {
                ...
                if (!elFocus && newActiveCell) {
                    newActiveCell.tabIndex = this._orgTabIndex;
                }
                if (oldActiveCell && oldActiveCell != newActiveCell) {
                    oldActiveCell.tabIndex = -1;
                }
                // make sure the grid didn't scroll out of whack in Safari/Mac (TFS 305904)
                this._fixScroll();
            ...
            // make sure non-scrollable elements didn't scroll (TFS 265197, 310092)
            FlexGrid.prototype._fixScroll = function () {
                ...
                [host, rootParent].forEach(function (e) {
                    if (e && e.scrollTop)
                        e.scrollTop = 0;
                    // don't set scroll left because it breaks in Chrome/RTL mode: TFS 311699
                    //if (e && e.scrollLeft && !this._rtlDir) e.scrollLeft = 0;
                });
            };
                ...
                if (!this.frozenRows && !this.frozenColumns) {
                    ...
                    if (!this.activeEditor) {
                        for (var i = 0; i < frozen.length; i++) {
                            ...
                            //cell.style.pointerEvents = 'auto'; // TFS 324182
            ...
            FlexGrid.controlTemplate = '<div style="position:relative;width:100%;height:100%;overflow:hidden;max-width:inherit;max-height:inherit">' +
                '<div wj-part="root" style="position:absolute;width:100%;height:100%;overflow:auto;max-width:inherit;max-height:inherit;-webkit-overflow-scrolling:touch;">' + // cell container
                ...
                '</div>' +
                '</div>' +
                '<div wj-part="fcells" aria-hidden="true" class="wj-cells wj-frozen-clone" style="position:absolute;pointer-events:none;overflow:hidden"></div>' + // frozen cells
                ...
                '</div>' +
                '<div wj-part="focus" class="wj-cell" style="position:fixed;left:-32000px">0</div>' + // moved after the grid to keep Narrator happy
                '<div wj-part="sz" style="position:relative;visibility:hidden"></div>' + // auto size element
                '</div>';
            return FlexGrid;
        }(wijmo.Control));
    ...
    (function (grid) {
        ...
        var GridPanel = /** @class */ (function () {
            ...
            GridPanel.prototype._updateContent = function (recycle, state, offsetY) {
                ...
                var vrb = vru;
                if (vrb.isValid) {
                    var maxVal = 10000000, touching = g.isTouching, xtraRows = (rows.length <= g._vtRows) ? maxVal : (touching) ? 6 : 0, xtraCols = (cols.length <= g._vtCols) ? maxVal : (touching) ? 1 : 0;
                    vrb = new grid.CellRange(Math.max(vru.row - xtraRows, rows.frozen), Math.max(vru.col - xtraCols, cols.frozen), Math.min(vru.row2 + xtraRows, rows.length - 1), Math.min(vru.col2 + xtraCols, cols.length - 1));
                }
            ...
            GridPanel.prototype._renderCell = function (row, r, c, rng, state, ctr) {
                ...
                if (cell && state) {
                    ...
                    wijmo.setAttribute(cell, 'aria-selected', selState != ss.None || activeCell);
                    if (activeCell) {
            ...
            GridPanel.prototype._getViewRange = function () {
                ...
                if (this._ct == CellType.Cell || this._ct == CellType.RowHeader) {
                    ...
                    rng.row2 = rows.getItemAt(y + h); // + 1); // TFS 273979, 316887
                ...
                if (this._ct == CellType.Cell || this._ct == CellType.ColumnHeader) {
                    ...
                    rng.col2 = cols.getItemAt(x + w); // + 1); // TFS 273979, 316887
                }
    ...
    (function (grid) {
        ...
        var CellFactory = /** @class */ (function () {
            ...
            CellFactory.prototype.updateCell = function (p, r, c, cell, rng, updateContent) {
                var g = p.grid, rtl = g.rightToLeft, ct = grid.CellType, rows = p.rows, cols = p.columns, row = rows[r], col = cols[c], r2 = r, c2 = c, gr = (row instanceof grid.GroupRow ? row : null), nr = (row instanceof grid._NewRowTemplate ? row : null), cellWidth = col.renderWidth, cellHeight = row.renderHeight, cellType = p.cellType, cl = 'wj-cell', css = { display: '' };
                ...
                if (selState != ss.None && // selected states don't apply to
                    cellType == ct.Cell && !checkBox && // scrollable cells without checkboxes
                ...
                css.textAlignLast = css.textJustify = '';
                if (css.textAlign == 'justify-all') {
                    css.textAlign = css.textAlignLast = 'justify';
                    css.textJustify = 'distribute'; //'inter-ideograph';
                }
                // padding (TFS 229194)
                ...
                if (updateContent != false) {
                    ...
                    if (cellType == ct.Cell && c == g.columns.firstVisibleIndex &&
                        gr && gr.hasChildren && !this._isEditingCell(g, r, c)) {
                        // collapse/expand outline
                        var btn = this._getTreeBtn(gr);
                        content = wijmo.escapeHtml(content) || gr.getGroupHeader();
                        cell.innerHTML = btn.outerHTML + ' ' + content;
                        css.textAlign = '';
                    }
                    else if (cellType == ct.ColumnHeader && bcol.currentSort && g.showSort && (r2 == g._getSortRowIndex() || bcol != col)) {
                    ...
                    else if (cellType == ct.RowHeader && c == g.rowHeaders.columns.length - 1 && !content) {
                    ...
                    else if (cellType == ct.Cell && this._isEditingCell(g, r, c)) {
                        ...
                        cell.innerHTML = (bcol.multiLine || row.multiLine) && inpType != 'checkbox'
                        ...
                        // regular content
                        if (cellType == ct.Cell && (row.isContentHtml || bcol.isContentHtml)) {
                    ...
                    // c) is editable
                    if (cellType == ct.Cell && wijmo.input && bcol.dataMap && g.showDropDown &&
                        bcol.showDropDown != false && g.canEditCell(r, c)) {
                        // create icon once
                        if (!CellFactory._ddBtn) {
                            var cls = CellFactory._WJC_DROPDOWN, btn = wijmo.createElement('<button class="wj-btn wj-btn-glyph wj-right ' + cls + '" type="button" tabindex="-1">' +
                            ...
                            CellFactory._ddBtn = btn;
                        }
                        // append to cell (last child, TFS 323622, 323934, 325997)
                        var dd = CellFactory._ddBtn.cloneNode(true);
                        cell.appendChild(dd);
                    }
                }
            ...
            CellFactory.prototype._getTreeBtn = function (gr) {
...
(function (wijmo) {
    ...
    (function (grid_1) {
        ...
        // extends wijmo.controlBaseClass
        /**
        ...
        var Column = /** @class */ (function (_super) {
            ...
            Object.defineProperty(Column.prototype, "dataType", {
                ...
                set: function (value) {
                    value = wijmo.asEnum(value, wijmo.DataType);
                    if (this._type != value) {
                        this._type = value;
                        if (this.grid) {
                            this.grid.invalidate();
            ...
            Object.defineProperty(Column.prototype, "currentSort", {
                 ...
                 */
                get: function () {
                    var view = this.grid ? this.grid.collectionView : null, sds = view ? view.sortDescriptions : null, prop = (sds && sds.length) ? this._getBindingSort() : null;
                    if (prop) {
                        for (var i = 0; i < sds.length; i++) {
        ...
        var GroupRow = /** @class */ (function (_super) {
            ...
            GroupRow.prototype.getGroupHeader = function () {
                var grid = this.grid, fmt = grid.groupHeaderFormat || wijmo.culture.FlexGrid.groupHeaderFormat, group = wijmo.tryCast(this.dataItem, wijmo.collections.CollectionViewGroup);
                if (group && fmt) {
                    // get group info
            ...
            GroupRow.prototype._setCollapsed = function (collapsed) {
                ...
                var e = new grid_1.CellRangeEventArgs(g.cells, new grid_1.CellRange(this.index, -1));
    ...
    (function (grid_1) {
        ...
        var HitTestInfo = /** @class */ (function () {
            ...
            function HitTestInfo(grid, pt) {
                ...
                if (grid instanceof Element && !(grid instanceof grid_1.FlexGrid)) {
    ...
    (function (grid) {
        ...
        var DataMap = /** @class */ (function () {
            ...
            DataMap.prototype.getKeyValue = function (displayValue) {
                var displayPath = this._displayPath, index = this._indexOf(displayValue, displayPath, true); // case-sensitive first
                if (index < 0) {
                    index = this._indexOf(displayValue, displayPath, false); // try insensitive
                }
    ...
    (function (grid) {
        ...
        var _KeyboardHandler = /** @class */ (function () {
            ...
            /*private*/ _KeyboardHandler.prototype._keydown = function (e) {
                ...
                // ignore defaultPrevented if the target is an InputElement (TFS 202913, 272449)
            ...
            _KeyboardHandler.prototype._findNext = function (row, col) {
                ...
                for (var off = 0; off < cnt; off++) {
                    var index = (row + off) % cnt, txt = g.getCellData(index, col, true).trim().toLowerCase();
                                    ...
                                    if (!bcol.getIsRequired() && !bcol.isReadOnly) {
                                        if (g.getCellData(sr, sc, true)) {
                                            ...
                                            if (g.onBeginningEdit(e)) {
                                                if (ecv) {
                                                    var item = ecv.currentEditItem;
                                                    if (!ecvUpdating_1 && item && item != row.dataItem) {
                                                        ecvUpdating_1 = true;
                                                        ecv.beginUpdate();
                                                    }
                                                    item = row.dataItem;
    ...
    (function (grid) {
        ...
        var _MouseHandler = /** @class */ (function () {
            ...
            function _MouseHandler(g) {
                ...
                g.addEventListener(host, 'mousedown', function (e) {
                    ...
                    if (!e.defaultPrevented && e.button == 0) {
                        ...
                        // sanity
                        var doc = document;
                        g.removeEventListener(doc, 'mousemove');
                        g.removeEventListener(doc, 'mouseup');
                        // connect
                        g.addEventListener(doc, 'mousemove', mouseMove);
                        g.addEventListener(doc, 'mouseup', mouseUp);
                        // mouse is down
                        _this._isDown = true;
                        _this._mousedown(e);
                    }
            ...
            _MouseHandler.prototype._mousedown = function (e) {
                ...
                var g = this._g, ht = g.hitTest(e), ct = ht.cellType, ctrlKey = e.ctrlKey || e.metaKey, target = e.target;
            ...
            _MouseHandler.prototype._handleClick = function (e) {
                var g = this._g, target = e.target, ht = new grid.HitTestInfo(target, null), panel = ht.panel;
            ...
            _MouseHandler.prototype._mouseSelect = function (e, extend) {
                ...
                if (e && this._htDown && this._htDown.panel && g.selectionMode != grid.SelectionMode.None) {
                    ...
                    if (!wijmo.isIE9() && e.button >= 0 && e.target != g._root) {
                        ...
                        if (ht.cellType != grid.CellType.Cell && ht.cellType != grid.CellType.RowHeader && this._eMouse) {
                            setTimeout(function () {
                                if (_this._eMouse) {
            ...
            _MouseHandler.prototype._dragstart = function (e) {
                var g = this._g, ht = this._htDown, allowDragging = AllowDragging;
                ...
                if (!this._szRowCol) {
                    ...
                    else if (ht.cellType == grid.CellType.TopLeft && (g.allowDragging & allowDragging.Columns) &&
                        ht.col > -1 && g.topLeftCells.columns[ht.col].allowDragging) {
                        if (g.onDraggingColumn(args)) {
                            this._dragSrc = e.target;
                        }
                        else {
            ...
            _MouseHandler.prototype._dragover = function (e) {
                var g = this._g, ht = g.hitTest(e), htDown = this._dragSrc ? this._htDrag : null, // TFS 332066
                cellType = grid.CellType, valid = false;
                ...
                if (htDown && g.autoScroll) {
                    var rc = g._rcBounds, sp = g.scrollPosition, edge = wijmo.Control._DRAG_SCROLL_EDGE, step = wijmo.Control._DRAG_SCROLL_STEP;
            ...
            _MouseHandler.prototype._showResizeMarker = function (sz) {
                ...
                var ct = this._szArgs.panel.cellType, cellType = grid.CellType, css;
                if (this._szRowCol instanceof grid.Column) {
                    css = {
            ...
            _MouseHandler.prototype._showDragMarker = function (ht) {
                ...
                // update marker position
                var cellType = grid.CellType, css = {
                    left: 0,
                    top: 0,
                    width: 6,
                    height: 6,
...
(function (wijmo) {
    ...
    (function (grid) {
        'use strict';
        var _WJC_CHECKBOX = 'wj-cell-check'; // identifier for checkbox
        var _PAGE_SIZE = 4; // items per page on drop-down listbox
        /**
        ...
        var _EditHandler = /** @class */ (function () {
            ...
            _EditHandler.prototype.startEditing = function (fullEdit, r, c, focus, evt) {
                ...
                if (edt) {
                    ...
                    if (edt.type == 'checkbox') {
                        ...
                        var nf = wijmo.culture.Globalize.numberFormat, pct = nf['%'] || '%';
                        if (wijmo.isNumber(g.getCellData(r, c, false)) && edt.value.indexOf(pct) > -1) {
                            var val = edt.value, start = 0, end = val.length;
                            while (end > 0 && val[end - 1] == pct)
                                end--;
                            while (start < end && val[start] == pct)
                                start++;
            ...
            _EditHandler.prototype.setClipString = function (text, rng) {
                ...
                // expand range and raise Pasting event
                if (autoRange && !rng.isSingleCell && rows.length) {
                    this._expandClipRows(rows, rng);
                }
                var rngPaste = new grid.CellRange(rng.topRow, rng.leftCol, rng.topRow + rows.length - 1, rng.leftCol + rows[0].length - 1), e = new grid.CellRangeEventArgs(g.cells, rngPaste);
                if (!g.onPasting(e)) {
                    return false;
                }
                ...
                // copy lines to rows
                var ecv = g.editableCollectionView, row = rng.topRow, pasted = false;
                g.deferUpdate(function () {
                    ...
                    for (var i = 0; i < rows.length && row < g.rows.length; i++, row++) {
                        ...
                        var cells = rows[i], colIndex = rng.leftCol;
                        for (var j = 0; j < cells.length && colIndex < g.columns.length; j++, colIndex++) {
                            // skip invisible column, keep clip cell
                            var col = g.columns[colIndex];
                            if (!col.isVisible) {
                                j--;
                                continue;
                            }
                            // assign cell
                            if (_this._allowEditing(row, colIndex)) {
                                // honor maxLength (TFS 318980)
                                var data = cells[j];
                                if (col.maxLength) {
                                    data = data.substr(0, col.maxLength);
                                }
                                // raise events so user can cancel the paste
                                var eCell = new grid.CellRangeEventArgs(g.cells, new grid.CellRange(row, colIndex), data);
                                if (g.onPastingCell(eCell)) {
                                    if (ecv) {
                                        ecv.editItem(g.rows[row].dataItem);
                                        _this._edItem = ecv.currentEditItem; // TFS 267863
                                    }
                                    if (g.cells.setCellData(row, colIndex, eCell.data)) {
                    ...
                    // select paste range and raise pasted event
                    g.select(rngPaste);
                    g.onPasted(e);
                });
            };
            ...
            /*private*/ _EditHandler.prototype._commitRowEdits = function () {
                ...
                if (this.finishEditing() && this._edItem) {
                    ...
                    if (ecv) {
                        if (ecv.currentEditItem || ecv.currentAddItem) {
            ...
            /*private*/ _EditHandler.prototype._keydown = function (e) {
                ...
                switch (e.keyCode) {
                    ...
                    case wijmo.Key.Enter:
                        ...
                        if (edt && e.altKey) {
                            var edt_1 = e.target; // not necessarily our activeEditor...
                            if (edt_1 instanceof HTMLTextAreaElement && edt_1.wrap == 'soft') {
                                var text = edt_1.value, start = edt_1.selectionStart, end = edt_1.selectionEnd;
                                edt_1.value = text.substr(0, start) + '\n' + text.substr(end);
                                wijmo.setSelectionRange(edt_1, start + 1);
                            }
                            return true;
                        }
                        // let grid handle key if editing finished
            ...
            /*private*/ _EditHandler.prototype._keypress = function (e) {
                ...
                if (edt && edt.type != 'checkbox' &&
                    ...
                    // find string (case-sensitive first, TFS 311184)
                    var index = this._findString(this._list, text, true);
                    if (index < 0) {
                        index = this._findString(this._list, text, false);
                    }
                    if (index > -1) {
                        edt.value = this._list[index];
            ...
            _EditHandler.prototype._createListBox = function () {
                var _this = this;
                var g = this._g, rng = this._rng, row = g.rows[rng.row], col = g._getBindingColumn(g.cells, rng.row, g.columns[rng.col]), div = document.createElement('div');
                // just in case
                this._removeListBox();
                // create ListBox
                wijmo.addClass(div, 'wj-dropdown-panel wj-grid-listbox');
                wijmo.addClass(div, col.dropDownCssClass);
                var lbx = new wijmo.input.ListBox(div, {
                    maxHeight: row.renderHeight * 4,
                    isContentHtml: col.isContentHtml,
                    itemsSource: col.dataMap.getDisplayValues(row.dataItem),
                    selectedValue: g.activeEditor
    ...
    (function (grid) {
        ...
        var _ImeHandler = /** @class */ (function () {
            ...
            function _ImeHandler(g) {
                this._compStartBnd = this._compositionstart.bind(this); // TFS 312148
                this._updateImeFocusBnd = this._updateImeFocus.bind(this);
                ...
                g.cells.hostElement.parentElement.appendChild(tbx);
                this._updateImeFocus();
                // attach event handlers
                var host = g.hostElement, add = g.addEventListener.bind(g);
                add(tbx, 'compositionstart', this._compStartBnd);
                add(host, 'blur', this._updateImeFocusBnd);
                add(host, 'focus', this._updateImeFocusBnd);
                add(host, 'mousedown', this._mouseDownBnd, true);
                add(host, 'mouseup', this._mouseUpBnd, true);
            ...
            _ImeHandler.prototype.dispose = function () {
                var g = this._g, host = g.hostElement, rmv = g.removeEventListener.bind(this);
                // remove event listeners
                rmv(this._tbx, 'compositionstart', this._compStartBnd);
                rmv(host, 'blur', this._updateImeFocusBnd);
                rmv(host, 'focus', this._updateImeFocusBnd);
                rmv(host, 'mousedown', this._mouseDownBnd);
                rmv(host, 'mouseup', this._mouseUpBnd);
            ...
            _ImeHandler.prototype._compositionstart = function () {
                ...
                if (g.activeEditor == null) {
                    ...
                    if (g.startEditing(false, sel.row, sel.col, false) && g.activeEditor) {
                        // adjust for merged cells (TFS 270497)
                        sel = g.editRange;
                        // get editor position
                        var edt_1 = g.activeEditor, tbx_1 = this._tbx, host = g.cells.hostElement, left = g.columns[sel.col].pos + host.offsetLeft, top_1 = g.rows[sel.row].pos + host.offsetTop, rc = g.getCellBoundingRect(sel.row, sel.col), cell = edt_1.parentElement, csCell = getComputedStyle(cell), zIndex = cell.style.zIndex;
                        if (!sel.isSingleCell) {
                        ...
                        // allow wrapping only if the editor is a textarea
                        wijmo.setAttribute(tbx_1, 'wrap', edt_1 instanceof HTMLTextAreaElement ? 'soft' : 'off');
                        // empty old editor (TFS 332598)
                        edt_1.value = '';
            ...
            _ImeHandler.prototype._updateImeFocus = function () {
                ...
                if (!g.activeEditor && !g.isTouching && !this._isMouseDown &&
                    wijmo.closest(ae, '.wj-flexgrid') == g.hostElement) {
                    var tbx = this._tbx;
                    if (this._enableIme()) {
                        if (ae != tbx) {
                            tbx.disabled = false;
