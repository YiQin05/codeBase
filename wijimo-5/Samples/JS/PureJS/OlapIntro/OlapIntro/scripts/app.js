...
onload = function () {
    ...
    // define list of products and countries
    app.products = 'Wijmo,Aoba,Olap,Xuni'.split(',');
    app.countries = 'US,Germany,UK,Japan,Italy,Greece,Spain,Portugal'.split(',');
...
function getSimpleDataSet(cnt) {
    var dtOct = new Date(2015, 9, 1),
        ...
        data = [
            { product: app.products[0], country: app.countries[0], sales: 10, downloads: 100, date: dtOct, active: true },
            { product: app.products[0], country: app.countries[1], sales: 10, downloads: 100, date: dtOct, active: false },
            { product: app.products[1], country: app.countries[0], sales: 10, downloads: 100, date: dtDec, active: true },
            { product: app.products[1], country: app.countries[1], sales: 10, downloads: 100, date: dtDec, active: false }
        ];
    for (var i = 0; i < cnt - 4; i++) {
        data.push({
            product: randomInt(1) ? app.products[0] : app.products[1],
            country: randomInt(1) ? app.countries[0] : app.countries[1],
            active: i % 2 == 0,
...
function getDataSet(cnt) {
    ...
    for (var i = 0; i < cnt; i++) {
        data.push({
            id: i,
            product: app.products[randomInt(app.products.length - 1)],
            country: app.countries[randomInt(app.countries.length - 1)],
            date: new Date(2015 + randomInt(2), randomInt(11), randomInt(27) + 1),
...
var urlNWind = 'https://services.odata.org/Northwind/Northwind.svc';
