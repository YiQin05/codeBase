    ...
    (function (grid) {
        ...
        (function (filter_1) {
            ...
            var FilterPanel = /** @class */ (function (_super) {
                ...
                // ** event handlers
                // remove filter on click (and raise filterChanging/Changed events)
                FilterPanel.prototype._click = function (e) {
                    var target = e.target;
                    if (target.classList.contains('wj-remove')) {
                        var marker = wijmo.closest(target, '.wj-filtermarker'), filter = this._filter, colFilter = marker ? marker['filter'] : null, col = colFilter instanceof filter_1.ColumnFilter ? colFilter.column.index : -1, evt = new grid.CellRangeEventArgs(filter.grid.cells, new grid.CellRange(-1, col));
                        if (filter.onFilterChanging(evt)) {
                            if (colFilter instanceof filter_1.ColumnFilter) {
                                colFilter.clear();
                                filter.apply();
                            }
                            else {
