<template>
    <div class="container inquire">
        ...
        <el-tab-pane label="下载图片">
          ...
          <el-button size="small" type="primary" @click="downLoad">下载原始图片</el-button>
        </el-tab-pane>
        <el-tab-pane label="导出特征矩阵">
          <el-button size="small" type="primary" @click="downLoadMatrix">导出已完成任务特征矩阵</el-button>
          <el-button size="small" type="primary" @click="downLoadSimilar">导出已完成任务特征相似度</el-button>
        </el-tab-pane>
      </el-tabs>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    downLoadMatrix () {
      let path = this.setAPIPath('/featureMatrix')
      this.$axios({
        method: 'get',
        url: path,
        responseType: 'blob'
        ...
        if (res.data.type === 'application/octet-stream') {
          ...
          link.style.display = 'none'
          link.href = url
          link.setAttribute('download', 'featureMatrix.csv')
          document.body.appendChild(link)
          link.click()
          this.openMsg('导出成功', 'success', this)
        } else {
          this.openMsg('导出失败', 'error', this)
        }
      })
    },
    downLoadSimilar () {
      let path = this.setAPIPath('/featureSimilar')
      this.$axios({
        method: 'get',
        url: path,
        responseType: 'blob'
        ...
        if (res.data.type === 'application/octet-stream') {
          ...
          link.style.display = 'none'
          link.href = url
          link.setAttribute('download', 'featureSimilar.csv')
          document.body.appendChild(link)
          link.click()
          this.openMsg('导出成功', 'success', this)
        } else {
