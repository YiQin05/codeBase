    ...
    <div>
        ...
        size="small">
            ...
            <template slot-scope="scope">
                ...
                size="mini"
                type="primary"
                @click="handleEdit(scope.$index, scope.row)" :disabled="(userType !== '2')? false:true">编辑</el-button>
                <el-button
                size="mini"
                type="danger"
                @click="handleDelete(scope.$index, scope.row)" :disabled="(userType !== '2')? false:true">删除</el-button>
            </template>
            </el-table-column>
        </el-table>
        <el-dialog title="小区编辑" :visible.sync="visible">
          <div class="cellEdit inputControl">
            <el-form :inline="true" ref="form1" :model="editCell" :rules="rules" class="demo-form-inline demo-ruleForm" size="mini" label-width="120px" style="text-align:left">
                <!-- string类型 -->
...
export default {
    ...
    return {
      ...
      cellMapEvents: {
        'click': (e) => {
          ...
          this.editCell.longitude = Math.ceil(longitude * 100000) / 100000
          this.editCell.latitude = Math.ceil(latitude * 100000) / 100000
  ...
  methods: {
    ...
    handleEdit (index, row) {
      ...
      // 第一个参数是latitude，第二个是longitude
      let change = this.wgsToGcj(row.latitude, row.longitude)
      let longitude = change.lon
      let latitude = change.lat
      this.positionCell = [longitude, latitude]
      this.visible = true
    ...
    handleDelete (index, row) {
      this.$confirm('此操作将永久该小区, 是否继续?', '提示', {
      ...
      this.$axios
        ...
        .then(response => {
          if (response.data.code === 0) {
            ...
            this.openMsg('删除失败，该小区已有任务，请先删除任务在执行删除小区操作！', 'error', this)
          } else {
            this.errorMsg(response.data.code, this)
          }
        })
