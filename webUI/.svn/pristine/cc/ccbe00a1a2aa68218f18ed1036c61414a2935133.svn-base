<template>
  <div class="container inputControl">
    <el-form :inline="true" ref="form1" :model="addCellList" :rules="rules" class="demo-form-inline demo-ruleForm" size="mini" label-width="80px" style="text-align:left">
        <!-- string类型 -->
...
export default {
  data () {
    ...
    return {
      ...
      // amapManager,
      isOpenMap: true,
      positionCell: [113.58811, 22.23263], // [113.564221, 22.370364]
      addCellMapEvents: {
        'click': (e) => {
          // 第一个参数是小latitude，第二个是大longitude
  ...
  watch: {
    takePhotoLongitude (newVal, oldVal) {
      if (newVal) {
        ...
        let longitude = change.lon
        let latitude = change.lat
        this.positionCell = [longitude, latitude]
      }
    },
    ...
    bscID (newVal, oldVal) {
      ...
      if (JSON.stringify(obj) !== '{}') {
        this.addCellList.bscName = obj.bscName
        this.addCellList.longitude = obj.longitude
        this.addCellList.latitude = obj.latitude
        this.addCellList.altitude = obj.altitude
      } else {
        this.addCellList.bscName = ''
        this.addCellList.longitude = ''
        this.addCellList.latitude = ''
        this.addCellList.altitude = ''
      }
    }
  },
  computed: {
    bscID () {
      return this.addCellList.bscID
    },
    takePhotoLongitude () {
  ...
  methods: {
    ...
    cellSubmit () {
      ...
      params.append('coverAngleBegin', 0)
      params.append('coverAngleEnd', 0)
      params.append('coverDistance', parseInt(this.addCellList.coverDistance))
      for (let key in this.addCellList) {
        console.log(`${key}: ` + this.addCellList[key])
      }
      this.$axios
        .post(path, params)
        .then(response => {
...
@media screen and (min-width: 1334px) {
  .el-form--inline .single {
    margin-right: 325px;
    -moz-margin-right: 313px;
    -webkit-margin-right: 325px;
  }
}
