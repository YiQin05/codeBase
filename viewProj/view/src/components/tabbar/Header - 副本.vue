<template >
    <div>
        ...
        <div class="container navi">
          <!-- to="/missionDetail/:userID"
          to="/scene/:userID"
          to="/inquire/:userID"
          to="/system/:userID"
          to="/home/login/:userID"
           -->
            <el-menu :default-active="activeIndex" active-text-color="white" class="menuWidth" mode="horizontal" @select="handleSelect">
                <el-menu-item index="1"><router-link :to="{ name: 'home', params: { userID: userID }}" tag="a">首页</router-link></el-menu-item>
                <el-menu-item index="2"><router-link :to="{ name: 'missionDetail', params: { userID: userID }}" tag="a">任务管理</router-link></el-menu-item>
                <el-menu-item index="3"><router-link :to="{ name: 'scene', params: { userID: userID }}" tag="a">场景识别</router-link></el-menu-item>
                <el-menu-item index="4"><router-link :to="{ name: 'inquire', params: { userID: userID }}" tag="a">数据管理</router-link></el-menu-item>
                <el-menu-item index="5"><router-link :to="{ name: 'system', params: { userID: userID }}" tag="a">系统管理</router-link></el-menu-item>
                <div id="log" :class="{'show':flag}">
                  <div class="login"><router-link :to="{ name: 'login', params: { userID: userID }}" tag="a">登录</router-link></div>
                </div>
...
export default {
  ...
  data () {
    return {
      ...
      userID: ':use123' //  :userID为原始未登录路由路径
    }
  },
  mounted: function () {
  ...
  methods: {
    handleSelect (key) {
      ...
      switch (key) {
        ...
        case '2':
          ...
          this.$router.push('/missionDetail/' + this.userID)
          this.activeIndex = '2'
          break
        case '3':
          this.setUrlCookie('navi', '3', expireDays)
          this.$router.push('/scene/' + this.userID)
          this.activeIndex = '3'
          break
        case '4':
          this.setUrlCookie('navi', '4', expireDays)
          this.$router.push('/inquire/' + this.userID)
          this.activeIndex = '4'
          break
        case '5':
          this.setUrlCookie('navi', '5', expireDays)
          this.$router.push('/system/' + this.userID)
          this.activeIndex = '5'
          break
      }
    },
    logOut () {
      ...
      this.delCookie('navi')
      this.flag = false
      this.$router.push('/home/login/:userID')
    }
  }
}
</script>
