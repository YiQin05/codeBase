    ...
    <div>
        ...
        <div class="container navi">
            <el-menu :default-active="activeIndex" active-text-color="white" class="menuWidth" mode="horizontal" @select="handleSelect">
                ...
                <el-menu-item index="6"><router-link :to="{ name: 'apply', params: { userID: userID }}" tag="a">应用案例</router-link></el-menu-item>
...
export default {
  ...
  methods: {
    active (activeUrl) {
      let arr = ['home', 'missionDetail', 'scene', 'inquire', 'system', 'apply']
      for (let i in arr) {
        if (activeUrl.indexOf(arr[i]) !== -1) {
          let k = (++i).toString()
          this.activeIndex = k
          break
        }
